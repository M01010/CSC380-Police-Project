...
videos_blueprint = Blueprint('videos', __name__, template_folder='templates', url_prefix='/videos')
...
@videos_blueprint.route('/<int:video_id>')
def view_video(video_id):
    try:
        video = DB_Videos.get_video(video_id)
        body_cam = DB_BodyCam.get_body_cam(video['BODY_CAM_body_cam_id'])
        return render_template('view_video.html', video=video, body_cam=body_cam)
    except Exception as e:
        flash(f"Error: {e}", "danger")
        return redirect(url_for('videos.view_videos'))


@videos_blueprint.route('/delete/<int:video_id>')
...
@videos_blueprint.route('/add', methods=['GET', 'POST'])
def add_video():
    try:
        if request.method == 'GET':
            body_cams = DB_BodyCam.get_body_cams()
            return render_template('add_video.html', body_cams=body_cams, constants=Constants)
...
@videos_blueprint.route('/edit/<int:video_id>', methods=['GET', 'POST'])
def edit_video(video_id):
    try:
        if request.method == 'GET':
            ...
            body_cam = DB_BodyCam.get_body_cam(video['BODY_CAM_body_cam_id'])
            body_cams = DB_BodyCam.get_body_cams()
            return render_template('edit_video.html', video=video, body_cam=body_cam, body_cams=body_cams)
        DB_Videos.edit_video(video_id, request.form)
        flash("video updated successfully!", "info")
        return redirect(url_for('videos.view_video', video_id=video_id))
    except Exception as e:
        flash(f"Error: {e}", "danger")
        return redirect(url_for('videos.view_videos'))
